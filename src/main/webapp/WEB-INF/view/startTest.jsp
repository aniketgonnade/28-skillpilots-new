                                        ...
                                        <div class="container">
                                            <div class="aa">
                                                <ul id="nav-mobile" class="right hide-on-med-and-down">
                                                    <li><a href="#" style="color: #FFFFFF;">Test Name: ${test.testName}</a></li>
                                                    <li><a href="#" style="color: #FFFFFF;">Max Questions:${test.total_que}</a></li>
                                                    <li><a href="#" style="color: #FFFFFF;">Total Marks: ${test.total_mark}</a></li>
                                    <li><div>Time left: <span id="time"></span></div></li>
							...
							<div class="row">
								...
								<div class="col-xl-8 mb-30" style="height: 440px;">
									<div class="card-box  " id="style-3"
                                        ...
                                        <form id="testForm">
                                            <div id="question-container" style="border: 2px solid black; padding: 20px; margin-bottom: 20px;">
                                                <c:forEach var="question" items="${questions}" varStatus="loop">
                                                    <div class="question-container ${loop.index == 0 ? 'active' : ''}" id="question-${loop.index}" style="display: ${loop.index == 0 ? 'block' : 'none'};">
                                                        <p style="font-weight: 1000;">Question ${loop.index + 1}: ${question.text}</p>
                                                        <hr/>
                                                        <c:choose>
                                                            ...
                                                            <c:when test="${question.type eq 'MULTIPLE_CHOICE'}">
                                                                <p>
                                                                    <label>
                                                                        <input type="checkbox" name="answers[answer_${question.id}]" value="${question.option1}" />
                                                                        <span>${question.option1}</span>
                                                                    </label>
                                                                </p>
                                                                <p>
                                                                    <label>
                                                                        <input type="checkbox" name="answers[answer_${question.id}]" value="${question.option2}" />
                                                                        <span>${question.option2}</span>
                                                                    </label>
                                                                </p>
                                                                <p>
                                                                    <label>
                                                                        <input type="checkbox" name="answers[answer_${question.id}]" value="${question.option3}" />
                                                                        <span>${question.option3}</span>
                                                                    </label>
                                                                </p>
                                                                <p>
                                                                    <label>
                                                                        <input type="checkbox" name="answers[answer_${question.id}]" value="${question.option4}" />
                                                                        <span>${question.option4}</span>
                                                                    </label>
                                                                </p>
                                                            </c:when>
                                            ...
                                            <div class="btn-container">
                                                <button class="btn btn btn-primary waves-effect waves-light" type="button" onclick="showPrevQuestion()">Previous</button>
                                                <button class="btn btn btn-primary waves-effect waves-light" id="nextButton" type="button" onclick="showNextQuestion()">Next</button>
                                                <button class="btn btn btn-success waves-effect waves-light" type="button" onclick="submitTestForm(${studentId}, ${testId}, false)">Submit Test</button>
                                    ...
                                    <!-- Modal Structure -->
                                     <div id="resultModal" class="modal">
                                           ...
                                           <center>     <h4>Test Results</h4></center> <hr>
                                    ...
                                    <!--                 <p>Total Marks: <span id="totalMarks"></span></p>
                                     -->                <p>Correct Answers: <span id="correctAnswers"></span></p>
                                            ...
                                            <div class="modal-footer">
                                                <a href="assessment_test" class="modal-close waves-effect waves-green btn-flat">Close</a>
                                            </div>
                                        </div>
...
<script>
    ...
    window.addEventListener('blur', function() {
        if (!hasSubmitted) {
            isDisqualified = true;
            submitTestForm(${studentId}, ${testId}, true);
        }
    });
    ...
    window.addEventListener('focus', function() {
        if (isDisqualified) {
            Swal.fire({
                title: 'Disqualified!',
                text: 'You have been disqualified for switching windows.',
                icon: 'warning',
                confirmButtonText: 'OK'
            }).then(() => {
                window.location.href = "assessment_test";
            });
        }
    });  
    ...
    function submitTestForm(studentId, testId, disqualified = false) {
        ...
        .then(result => {
            ...
            Swal.fire({
                ...
                document.getElementById('marksObtained').innerText = result.marks;
                document.getElementById('correctAnswers').innerText = result.correctAns;
                document.getElementById('wrongAnswers').innerText = result.wrongAns;
                document.getElementById('unanswered').innerText = result.noAns;
                document.getElementById('resultStatus').innerText = result.resultStatus;
    
                var instance = M.Modal.getInstance(resultModal);
                instance.open();
            });
        })
        .catch(error => {
